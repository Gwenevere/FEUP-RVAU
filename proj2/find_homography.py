...
def draw_rating(rating, image, image2, rvec, tvec):
    for x in range(rating):
        width = min(image2.shape[1], image2.shape[0])*0.15
        draw_cube_title(image, rvec, tvec, image2.shape[1]*0.5, image2.shape[0]*0.5, width=width, z=x, zgap=width*0.2)

def create_trans_im(width, height):
...
def draw_cube_title(image, rvec, tvec, dx=0, dy=0, width=200, z=0, zgap=20):
    dx -= width/2
    dy -= width/2
...
def compute_frame(img2):
    ...
    if(homography is not None):
        ...
        textimg = create_trans_im(img1.shape[0], img1.shape[1])
